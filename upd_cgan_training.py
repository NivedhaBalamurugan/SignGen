...
def train_gan(generator, discriminator, word_vectors, skeleton_sequences, epochs=100, batch_size=32, patience=10):
    ...
    generator_optimizer = tf.keras.optimizers.Adam(
        learning_rate=0.00015, beta_1=0.5, beta_2=0.9)
    ...
    chunks_per_epoch = max(1, num_samples // MAX_SAMPLES_PER_BATCH +
                           (1 if num_samples % MAX_SAMPLES_PER_BATCH > 0 else 0))

    logging.info(
        f"Training with {num_samples} samples, {num_samples//batch_size + (1 if num_samples % batch_size > 0 else 0)} batches per epoch")
    ...
    def train_step(word_vector_batch, real_skeleton_batch):
        ...
        noise = tf.random.normal(
            [actual_batch_size, CGAN_NOISE_DIM], dtype=FP_PRECISION)
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            gp = gradient_penalty(
                discriminator, real_skeleton_batch, generated_skeleton, word_vector_expanded)
            disc_loss += 10.0 * gp
    ...
    for epoch in range(epochs):
        ...
        if combined_loss < best_loss:
            logging.info(
                f"Loss improved from {best_loss:.4f} to {combined_loss:.4f}")
            ...
            patience_counter += 1
            logging.info(
                f"No improvement in loss. Patience: {patience_counter}/{patience}")
            ...
            if patience_counter >= patience:
                ...
                if best_generator_weights is not None and best_discriminator_weights is not None:
                    ...
                    set_model_weights(
                        discriminator, best_discriminator_weights)
                break
...
def main():
    ...
    try:
        ...
        if success:
            ...
            kld_score = calculate_kld(
                real_skeletons.flatten(), generated_skeletons.flatten())
